 ...
 * @description: 
 * @updateInfo: 本次更新内容：
 * @Date: 2021-10-14 13:52:49
 * @LastEditTime: 2021-10-14 15:30:05
-->
<script setup>
...
onMounted(() => {
  let re = document.createElement('link')
  re.setAttribute('rel', 'resource')
  re.setAttribute('type', 'application/l10n')
  re.setAttribute('href', 'locale/locale.properties')
  document.getElementsByTagName('head')[0].appendChild(re)
  loadFile('viewer.css', 'css')
  loadFile('build/pdf.js', 'js')
  loadFile('viewer.js', 'js')
})

watch(() => props.loadUrl, (newval) => {
  console.log('newval: ', newval);
  if (newval) loadUrl(newval)
})

const loadUrl = (url) => {
  if (!PDFViewerApplication) {
    throw new Exception('PDFViewerApplication is not defined')
  }
